    ...
    try {
      ...
      concerts.forEach(concert => {
        ...
        concertElement.innerHTML = `
          ...
          <button onclick="showTicketsModal(${concert.id})">Билеты</button>
        `;
  ...
  .then(data => {
    ...
    if (Array.isArray(data)) {
      ...
      ticketsList.innerHTML = `
        <div class="ticket-item">
          <p><strong>Ряд:</strong> ${data.row}, <strong>Место:</strong> ${data.seat}</p>
          <p><strong>Имя:</strong> ${data.buyerName || "—"}</p>
          <p><strong>Email:</strong> ${data.buyerEmail || "—"}</p>
          <p><strong>Телефон:</strong> ${data.buyerPhone || "—"}</p>
          <button onclick="editTicket(${data.id})">Редактировать</button>
        </div>
      `;
    } else {
      ticketsList.innerHTML = "<p>Не удалось загрузить билеты. Ответ не является массивом или объектом.</p>";
    }
  })
  .catch(error => {
